...
export default function AssessmentsPage() {
  ...
  const fetchReport = async (assessmentId: string) => {
    try {
      const response = await fetch(`/api/assessments/report/${assessmentId}`);
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      const data = await response.json();
      setAiReport(data.message); // Assuming the report content is in 'message' for now
    } catch (error) {
  ...
  const handleAssessmentComplete = (type: string, result: DiagnosticResult) => {
    ...
    // Assuming the result object contains an ID that can be used to fetch the report
    // For now, using a placeholder ID or the type itself
    fetchReport(result.id || type); 
  }
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="text-muted-foreground">
          Take diagnostic assessments to help us understand your learning needs.
        </p>
      </header>

      <Tabs defaultValue="adhd">
        <TabsList>
          <TabsTrigger value="adhd">ADHD Assessment</TabsTrigger>
          <TabsTrigger value="dyslexia">Dyslexia Assessment</TabsTrigger>
        </TabsList>
        <TabsContent value="adhd" className="mt-4">
          <Card>
            <CardHeader>
              <CardTitle>ADHD Diagnostic Assessment</CardTitle>
              <CardDescription>
                This assessment helps identify traits associated with Attention-Deficit/Hyperactivity Disorder.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <AdhdAssessment />
        ...
        <TabsContent value="dyslexia" className="mt-4">
          <Card>
            <CardHeader>
              <CardTitle>Dyslexia Diagnostic Assessment</CardTitle>
              <CardDescription>
                This assessment helps identify traits associated with Dyslexia.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <DyslexiaAssessment />
